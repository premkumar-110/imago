...
const Orders = () => {
  ...
  const handleDelivered = (id,useremail)=>{
    const deliver = async ()=>{
      ...
      const response = await axios.post(`${process.env.REACT_APP_SERVER_URL}admin/setProductDelivered`,{id,useremail});
      if(response){
        const response1 = await axios.get(`${process.env.REACT_APP_SERVER_URL}admin/getPurchase`);
          ...
          {filteredProducts.map((item) => (
            item.purchases.map((items) => (
              <tr key={items._id}>
                ...
                <td>{items.address}</td>
                <td>
                  <button onClick={()=>{handleDelivered(items._id,item.useremail)}} style={{backgroundColor: items.delivered ? "green": "",width:'100%'}}>
                    {items.delivered ? "DELIVERED" : "DELIVER"} <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-truck" viewBox="0 0 16 16">
